...
csv_path = os.path.join(dir_path, 'dataset.csv')
...
ax1.bar(range(len(first_eigenvector)), np.abs(first_eigenvector), color='green', alpha=0.7)
...
ax2.bar(range(len(second_eigenvector)), np.abs(second_eigenvector), color='red', alpha=0.7)
